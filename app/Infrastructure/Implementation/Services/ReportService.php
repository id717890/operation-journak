        ...
        Excel::create($file_name, function ($excel) use ($incidents, $title) {
            $excel->sheet('Report', function ($sheet) use ($incidents, $title) {
                ...
                foreach ($incidents as $key => $value) {
                    $sheetArray[] = [$key+1, date('d.m.y H:i', strtotime($value->start_date)), !is_null($value->end_date) ? date('d.m.Y H:i',strtotime($value->end_date)) : '', $value->object_caption(), $value->issue, $value->actions, $value->other, $value->user->name];
                    $count_rows++;
                }
    ...
    public function export_journal_history_to_excel($incidents, $title)
    {
        ...
        Excel::create($file_name, function ($excel) use ($incidents, $title) {
            $excel->sheet('Report', function ($sheet) use ($incidents, $title) {
                ...
                foreach ($incidents as $key => $value) {
                    $sheetArray[] = [$key+1, date('d.m.y H:i', strtotime($value->start_date)), date('d.m.y H:i', strtotime($value->end_date)), $value->object_caption(), $value->issue, $value->actions, $value->other, $value->user->name];
                    $count_rows++;
                }
